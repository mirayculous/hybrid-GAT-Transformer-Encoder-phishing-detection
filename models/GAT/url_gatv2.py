# ==============================================================================
# PHISHING DETECTION — URL-ONLY MODEL (GATv2)
# Dataset : url.xlsx → kolom: Category (spam/ham), Data (URL)
# Platform: Kaggle Notebook (GPU T4 recommended)
# Ablation : Model tunggal URL saja menggunakan GATv2Conv (Brody et al., 2022)
#
# Perbedaan GATv2 vs GAT Standard:
#   GAT    : e_ij = LeakyReLU(a^T [Wh_i || Wh_j])      ← static attention
#   GATv2  : e_ij = a^T LeakyReLU(W [h_i || h_j])      ← dynamic attention
#   GATv2 lebih ekspresif karena attention dihitung SETELAH non-linearity
# ==============================================================================
...
from torch_geometric.nn import GATv2Conv, global_mean_pool, global_max_pool  # ← GATv2Conv
...
class Config:
    ...
    PROCESSED_DIR   = "/kaggle/working/processed_gatv2"
    MODEL_SAVE_PATH = "/kaggle/working/best_model_gatv2.pt"
    HISTORY_PATH    = "/kaggle/working/history_gatv2.json"
    ...
    # GATv2 — parameter identik dengan GAT Standard untuk perbandingan adil
    GAT_HIDDEN_DIM = 64
    GAT_OUT_DIM    = 128
    GAT_HEADS      = 4
    GAT_DROPOUT    = 0.3
    ...
    # Training — identik dengan GAT Standard
    BATCH_SIZE  = 32
    ACCUM_STEPS = 2
    EPOCHS      = 30
    LR          = 1e-3
...
class URLGATv2(nn.Module):
    """
    URL-only phishing detector menggunakan GATv2Conv (Brody et al., 2022).
    
    GATv2 memperbaiki kelemahan GAT standard ("static attention problem"):
      GAT   : α_ij ∝ LeakyReLU(a^T [Wh_i || Wh_j])
      GATv2 : α_ij ∝ a^T LeakyReLU(W[h_i || h_j])
    Dengan menempatkan non-linearity sebelum inner product, attention score
    bergantung penuh pada KOMBINASI (h_i, h_j) sehingga lebih ekspresif.
    
    Pipeline: Char-level embedding + 21 handcrafted features
              → node projection → 2-layer GATv2
    ...
    def __init__(self):
        ...
        for i in range(cfg.GAT_LAYERS):
            ...
            self.gat_layers.append(
                GATv2Conv(                          # ← GATv2Conv
                    in_dim, out_d, heads=heads,
                    concat=not last,
                    dropout=cfg.GAT_DROPOUT,
                    add_self_loops=True,
                    share_weights=False             # independent W_l, W_r
                ))
...
# ==============================================================================
# CELL 9 — Training Utilities (identik dengan GAT Standard)
# ==============================================================================
class EarlyStopping:
...
def train():
    ...
    model     = URLGATv2().to(cfg.DEVICE)
    total     = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel           : URL-only | GATv2")
...
def final_test(model, test_loader):
    print("\n" + "=" * 60)
    print("  FINAL TEST EVALUATION — URL-only | GATv2")
    print("=" * 60)
