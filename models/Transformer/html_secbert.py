# ==============================================================================
# PHISHING DETECTION — HTML-ONLY MODEL (SecBERT)
# Dataset : html.xlsx → kolom: Category (spam/ham), Data (HTML content)
# Platform: Kaggle Notebook (GPU T4 recommended)
# Ablation : Model tunggal HTML saja menggunakan jackaduma/SecBERT
#
# SecBERT (Jain et al. / jackaduma, 2021) — Security-focused BERT
#   • Fine-tuned dari BERT-base-uncased
#   • Pre-training corpus: teks domain keamanan siber (vulnerability reports,
#     CVE descriptions, security blogs, malware writeups, phishing datasets)
#   • Hipotesis KUAT: vocabulary dan representasi SecBERT SANGAT relevan
#     untuk phishing karena domain pre-training nya tepat sasaran
#   • Tokenizer: WordPiece (identik dengan BERT), hidden dim: 768
#   • Ini adalah model paling "in-domain" untuk tugas deteksi phishing
# ==============================================================================
...
from transformers import BertTokenizer, BertModel   # ← SecBERT dibangun di atas BERT
...
class Config:
    ...
    PROCESSED_DIR   = "/kaggle/working/processed_secbert"
    MODEL_SAVE_PATH = "/kaggle/working/best_model_secbert.pt"
    HISTORY_PATH    = "/kaggle/working/history_secbert.json"
    ...
    # Model — SecBERT (BERT-based, security domain)
    BERT_MODEL      = "jackaduma/SecBERT"
    BERT_HIDDEN_DIM = 768
    MAX_HTML_LEN    = 256
    STAT_FEAT_DIM   = 16
    HTML_PROJ_DIM   = 128
    ...
    # Training — karena sudah in-domain, LR bisa sedikit lebih agresif
    BATCH_SIZE  = 16
    ACCUM_STEPS = 4
    EPOCHS      = 30
    LR          = 2e-5
...
print(f"Model           : HTML-only | SecBERT ({cfg.BERT_MODEL})")
print(f"Domain          : Cybersecurity (phishing, malware, CVE)")
...
# Untuk SecBERT: sertakan keyword keamanan eksplisit agar memanfaatkan
# domain knowledge yang sudah ada di pre-training corpus SecBERT
# ==============================================================================
def extract_html_text(raw_html):
    ...
    try:
        ...
        for form in soup.find_all('form'):
            ...
            struct.append(f"form action {action[:80]}")
        # Input fields — keyword keamanan
        for inp in soup.find_all('input'):
            t   = inp.get('type', 'text').lower()
            nm  = inp.get('name', '').lower()
            struct.append(f"input {t} {nm}")
        # Hyperlinks
        for a in soup.find_all('a', href=True)[:20]:
            struct.append(f"link {a['href'][:80]}")
        # Script sources
        for sc in soup.find_all('script', src=True)[:10]:
            struct.append(f"script {sc['src'][:60]}")
        # iframes
...
# ==============================================================================
# CELL 9 — Model: HTML-only SecBERT
# ==============================================================================
class HTMLSecBERT(nn.Module):
    """
    HTML-only phishing detector menggunakan jackaduma/SecBERT.

    SecBERT adalah model BERT yang di-pretrain pada corpus cybersecurity:
    laporan kerentanan, deskripsi CVE, blog keamanan, writeup malware, dll.
    Secara teori, ini adalah model paling cocok untuk phishing detection
    karena domain pre-training-nya langsung relevan.
...
def train():
    ...
    model     = HTMLSecBERT().to(cfg.DEVICE)
    total     = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel           : HTML-only | SecBERT ({cfg.BERT_MODEL})")
    ...
    print("\n" + "=" * 60)
    print(f"  TRAINING: HTML-only | SecBERT")
    print("=" * 60)
...
def final_test(model, test_loader):
    print("\n" + "=" * 60)
    print(f"  FINAL TEST EVALUATION — HTML-only | SecBERT")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("  SELESAI! — HTML-only | SecBERT")
