# ==============================================================================
# PHISHING DETECTION — HTML-ONLY MODEL (CodeBERT)
# Dataset : html.xlsx → kolom: Category (spam/ham), Data (HTML content)
# Platform: Kaggle Notebook (GPU T4 recommended)
# Ablation : Model tunggal HTML saja menggunakan microsoft/codebert-base
#
# CodeBERT (Feng et al., 2020) — Pre-trained Model for Programming & Natural Language
#   • Arsitektur: RoBERTa-base (bukan BERT-base)
#   • Pre-training pada CodeSearchNet: 6 bahasa (Python, Java, JS, PHP, Ruby, Go)
#   • Dua tujuan pre-training: MLM + Replaced Token Detection (bimodal data)
#   • Sangat relevan untuk HTML karena HTML adalah bahasa markup/kode
#   • Tokenizer: BPE (byte-pair encoding), vocab 50.265, hidden dim: 768
#   • Hipotesis: struktur tag HTML, href, script src lebih dipahami CodeBERT
#     dibanding model NLP umum
# ==============================================================================
...
from transformers import RobertaTokenizer, RobertaModel   # ← CodeBERT dibangun di atas RoBERTa
...
class Config:
    ...
    PROCESSED_DIR   = "/kaggle/working/processed_codebert"
    MODEL_SAVE_PATH = "/kaggle/working/best_model_codebert.pt"
    HISTORY_PATH    = "/kaggle/working/history_codebert.json"
...
print(f"Model           : HTML-only | CodeBERT ({cfg.BERT_MODEL})")
...
# CELL 4 — Tokenizer
# ==============================================================================
# CodeBERT menggunakan RobertaTokenizer (BPE), bukan BertTokenizer (WordPiece)
tokenizer = RobertaTokenizer.from_pretrained(cfg.BERT_MODEL)
...
# Untuk CodeBERT: pertahankan struktur markup lebih banyak karena model
# terlatih memahami sintaks kode / markup
# ==============================================================================
def extract_html_text(raw_html):
    ...
    try:
        ...
        # Untuk CodeBERT: sertakan lebih banyak info struktural HTML/kode
        # karena model sudah familiar dengan sintaks markup
        for tag in soup(['style', 'noscript']): tag.decompose()

        # Teks biasa
        visible = re.sub(r'\s+', ' ', soup.get_text(separator=' ', strip=True)).lower().strip()

        struct = []
        # Form action — indikasi kuat phishing
        for form in soup.find_all('form'):
            struct.append(f"form action={form.get('action', '')[:80]}")
        # Input fields
        for inp in soup.find_all('input'):
            struct.append(f"input type={inp.get('type','text')} name={inp.get('name','')}")
        # Hyperlinks — sangat penting untuk phishing
        for a in soup.find_all('a', href=True)[:20]:
            struct.append(f"a href={a['href'][:80]}")
        # Script sources
        for sc in soup.find_all('script', src=True)[:10]:
            struct.append(f"script src={sc['src'][:60]}")
        # iframes — sering digunakan phishing
        for fr in soup.find_all('iframe', src=True)[:5]:
            struct.append(f"iframe src={fr['src'][:60]}")
        # Meta tags
        for mt in soup.find_all('meta')[:5]:
            if mt.get('content'): struct.append(f"meta {mt.get('name','')} {mt.get('content','')[:40]}")

        return visible[:400] + ' </s> ' + ' '.join(struct)[:300]
    except Exception:
        return "<pad>"
...
def tokenize_html(text):
    ...
    # RoBERTa menggunakan </s> sebagai separator
    if len(tokens) > cfg.MAX_HTML_LEN - 2:
        half   = (cfg.MAX_HTML_LEN - 3) // 2
        tokens = tokens[:half] + ['</s>'] + tokens[-half:]
...
# ==============================================================================
# CELL 9 — Model: HTML-only CodeBERT
# ==============================================================================
class HTMLCodeBERT(nn.Module):
    """
    HTML-only phishing detector menggunakan microsoft/codebert-base.

    CodeBERT (RoBERTa-based) pre-trained pada kode program multimodal.
    Hipotesis: pemahaman sintaks kode/markup CodeBERT membantu interpretasi
    struktur HTML — tag, href, script src — yang merupakan sinyal penting phishing.

    Output: CLS token [768-dim] + 16 stat features → fusion → classifier
    Frozen: semua layer kecuali transformer layer terakhir.
    """
    def __init__(self):
        super().__init__()
        self.bert = RobertaModel.from_pretrained(cfg.BERT_MODEL)

        # Freeze semua, unfreeze layer terakhir
        ...
        self.fusion = nn.Sequential(
            nn.Linear(cfg.BERT_HIDDEN_DIM + 128, 512),
            nn.LayerNorm(512), nn.ReLU(), nn.Dropout(cfg.DROPOUT),
            nn.Linear(512, cfg.HTML_PROJ_DIM))
...
def train():
    ...
    model     = HTMLCodeBERT().to(cfg.DEVICE)
    total     = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel           : HTML-only | CodeBERT ({cfg.BERT_MODEL})")
    ...
    print("\n" + "=" * 60)
    print(f"  TRAINING: HTML-only | CodeBERT")
    print("=" * 60)
...
def final_test(model, test_loader):
    print("\n" + "=" * 60)
    print(f"  FINAL TEST EVALUATION — HTML-only | CodeBERT")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("  SELESAI! — HTML-only | CodeBERT")
