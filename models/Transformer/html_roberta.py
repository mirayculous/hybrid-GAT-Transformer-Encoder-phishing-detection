# ==============================================================================
# PHISHING DETECTION — HTML-ONLY MODEL (RoBERTa)
# Dataset : html.xlsx → kolom: Category (spam/ham), Data (HTML content)
# Platform: Kaggle Notebook (GPU T4 recommended)
# Ablation : Model tunggal HTML saja menggunakan roberta-base
#
# RoBERTa (Liu et al., 2019) — Robustly Optimized BERT Pretraining Approach
#   • Pre-training lebih lama, batch lebih besar, tanpa NSP (Next Sentence Prediction)
#   • Dynamic masking vs static masking pada BERT
#   • Tokenizer: BPE (Byte-Pair Encoding), vocab 50K
#   • Hidden dim: 768, 12 layers, 12 heads
# ==============================================================================
...
class Config:
    ...
    PROCESSED_DIR   = "/kaggle/working/processed_roberta"
    MODEL_SAVE_PATH = "/kaggle/working/best_model_roberta.pt"
    HISTORY_PATH    = "/kaggle/working/history_roberta.json"
    ...
    # Training
    BATCH_SIZE  = 16
    ACCUM_STEPS = 4           # efektif batch = 16×4 = 64
    EPOCHS      = 30
    LR          = 2e-5
    WEIGHT_DECAY= 1e-4
    PATIENCE    = 5
...
print(f"Model           : HTML-only | RoBERTa ({cfg.BERT_MODEL})")
...
def extract_html_text(raw_html):
    ...
    try:
        ...
        return visible[:500] + ' </s> ' + ' '.join(struct)[:200]
    except Exception:
        return "<pad>"
...
# ==============================================================================
# CELL 9 — Model: HTML-only RoBERTa
# ==============================================================================
class HTMLRoBERTa(nn.Module):
    """
    HTML-only phishing detector menggunakan roberta-base.
    Pipeline: RoBERTa CLS token [768-dim]
              + 16 statistical/handcrafted HTML features [→128-dim]
              → fusion [→128-dim] → MLP classifier
    Frozen: semua layer RoBERTa kecuali transformer layer terakhir.
    """
    def __init__(self):
        super().__init__()
        self.bert = RobertaModel.from_pretrained(cfg.BERT_MODEL)

        # Freeze semua, unfreeze layer terakhir saja
...
def train():
    ...
    model     = HTMLRoBERTa().to(cfg.DEVICE)
    total     = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel           : HTML-only | RoBERTa ({cfg.BERT_MODEL})")
    ...
    print("\n" + "=" * 60)
    print(f"  TRAINING: HTML-only | RoBERTa")
    print("=" * 60)
...
def final_test(model, test_loader):
    print("\n" + "=" * 60)
    print(f"  FINAL TEST EVALUATION — HTML-only | RoBERTa")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("  SELESAI! — HTML-only | RoBERTa")
