# ==============================================================================
# PHISHING DETECTION — HTML-ONLY MODEL (DeBERTa-v3)
# Dataset : html.xlsx → kolom: Category (spam/ham), Data (HTML content)
# Platform: Kaggle Notebook (GPU T4 recommended)
# Ablation : Model tunggal HTML saja menggunakan microsoft/deberta-v3-base
#
# DeBERTa-v3 (He et al., 2021/2023) — Decoding-Enhanced BERT with Disentangled Attention
#   • Disentangled Attention: posisi & konten direpresentasikan secara terpisah
#   • Enhanced Mask Decoder (EMD): memperbaiki fine-tuning untuk token prediction
#   • v3: menggunakan ELECTRA-style pre-training (generator + discriminator)
#         → lebih efisien, performa SOTA pada banyak NLU benchmark
#   • Tokenizer: SentencePiece, hidden dim: 768, 12 layers
# ==============================================================================
...
!pip install sentencepiece -q     ← wajib untuk DeBERTa v3 tokenizer
...
from transformers import DebertaV2Tokenizer, DebertaV2Model   # ← DeBERTa v3
...
class Config:
    ...
    PROCESSED_DIR   = "/kaggle/working/processed_deberta"
    MODEL_SAVE_PATH = "/kaggle/working/best_model_deberta.pt"
    HISTORY_PATH    = "/kaggle/working/history_deberta.json"
    ...
    # Training — sedikit lebih konservatif karena DeBERTa lebih sensitif LR tinggi
    BATCH_SIZE  = 16
    ACCUM_STEPS = 4
    EPOCHS      = 30
    LR          = 1e-5          # ← lebih kecil dari RoBERTa/DistilBERT
    WEIGHT_DECAY= 1e-4
    PATIENCE    = 5
    GRAD_CLIP   = 1.0
...
print(f"Model           : HTML-only | DeBERTa-v3 ({cfg.BERT_MODEL})")
...
# CELL 4 — Tokenizer
# ==============================================================================
tokenizer = DebertaV2Tokenizer.from_pretrained(cfg.BERT_MODEL)
...
# ==============================================================================
# CELL 9 — Model: HTML-only DeBERTa-v3
# ==============================================================================
class HTMLDeBERTa(nn.Module):
    """
    HTML-only phishing detector menggunakan microsoft/deberta-v3-base.

    DeBERTa-v3 menggunakan Disentangled Attention (konten & posisi dipisah)
    dan ELECTRA-style pre-training, sehingga sangat kuat untuk tugas NLU.
    Output CLS token [768-dim] + 16 stat features → fusion → classifier.
    Frozen: semua layer kecuali transformer layer terakhir.
    """
    def __init__(self):
        super().__init__()
        self.bert = DebertaV2Model.from_pretrained(cfg.BERT_MODEL)
    ...
    def forward(self, input_ids, attention_mask, stat_feat):
        # DeBERTa v3: ambil token pertama sebagai representasi kalimat
...
def train():
    ...
    model     = HTMLDeBERTa().to(cfg.DEVICE)
    total     = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel           : HTML-only | DeBERTa-v3 ({cfg.BERT_MODEL})")
    ...
    print("\n" + "=" * 60)
    print(f"  TRAINING: HTML-only | DeBERTa-v3")
    print("=" * 60)
...
def final_test(model, test_loader):
    print("\n" + "=" * 60)
    print(f"  FINAL TEST EVALUATION — HTML-only | DeBERTa-v3")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("  SELESAI! — HTML-only | DeBERTa-v3")
